import { ChainId, WNATIVE_ADDRESS, getProviderOrSigner, toElastic, toShare } from '@telefy/teleswap-core-sdk'
...
export class KashiCooker {
  ...
  add(action: KashiAction, data: string, value: JSBI = JSBI.BigInt(0)): void {
  ...
  updateExchangeRate(mustUpdate = false, minRate = JSBI.BigInt(0), maxRate = JSBI.BigInt(0)): KashiCooker {
    this.add(
  ...
  bentoDepositCollateral(amount: JSBI): KashiCooker {
    ...
    this.add(
      ...
      useNative ? amount : JSBI.BigInt(0)
    )
  ...
  addCollateral(amount: JSBI, fromBento: boolean): KashiCooker {
    let share: JSBI
    if (fromBento) {
      share = JSBI.lessThan(amount, JSBI.BigInt(0)) ? amount : toShare(this.pair.collateral, amount)
    } else {
      const useNative = this.pair.collateral.address === WNATIVE_ADDRESS[this.chainId]
    ...
    try {
      return {
        ...
        tx: await kashiPairCloneContract.cook(this.actions, this.values, this.datas, {
          value: this.values.reduce((a, b) => JSBI.add(a, b), JSBI.BigInt(0)),
        }),
      }
    } catch (error) {
